...
class delete_compte
{
    ...
    {
        $sql = $GLOBALS['bdd']->prepare("SELECT password FROM membre WHERE email=:mail");
        $sql->bindParam(":mail", $this->mail, PDO::PARAM_STR);
        $sql->execute();
        $passe = $sql->fetch();
        if (password_verify($this->password, $passe['password'])) {
            $query = $GLOBALS['bdd']->prepare("UPDATE membre set compte_supprimer=\"delete\" WHERE email=:mail");
